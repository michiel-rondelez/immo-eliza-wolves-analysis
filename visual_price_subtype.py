...
properties_data = remove_outliers(properties_data, "Living area")[(properties_data["Price per m2"]<= threshold.astype(int)) & (properties_data["Type of property"] == "Appartment")]
#print(properties_data.head())

"""
#visual for price/m2/subtype, price cutoff #################################################################################################
order = properties_data.groupby("Subtype of property")["Price"].median().sort_values().index
threshold = properties_data["Price"].quantile(0.75)
sns.boxplot(x="Subtype of property", y="Price per m2", data=properties_data[properties_data["Price"]<= threshold.astype(int)], order=order, palette='Pastel2')
sns.set_style("whitegrid")
plt.axhline(properties_data["Price per m2"].median(), color='purple', linestyle='-', linewidth=3)
plt.yscale('log')
plt.show()
"""
...
order = properties_data.groupby("Subtype of property")["Living area"].median().sort_values().index
#threshold = properties_data["Price per m2"].quantile(0.90)
sns.boxplot(
...
median_area = properties_data.groupby("Subtype of property")["Living area"].median().sort_values()
